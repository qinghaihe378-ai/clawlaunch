...
async function testToken(tokenAddress: `0x${string}`, tokenName: string) {
  console.log('\n' + '='.repeat(60))
  console.log(`🧪 测试代币: ${tokenName} (滑点 5%)`)
  ...
  let balanceAfterBuy = BigInt(0)
  
  // 1. 买入 (0.0001 BNB)
  console.log('💰 步骤 1: 买入 (0.0001 BNB, 5% 滑点)')
  try {
    ...
    const amountOutMin = amountsOut[1] * BigInt(9500) / BigInt(10000) // 5% slippage
    console.log(`   最小获得 (5%): ${formatUnits(amountOutMin, Number(decimals))}`)
    ...
    const buyHash = await walletClient.writeContract({
      ...
      value: buyAmount,
      gas: BigInt(400000) // Increase gas for tax tokens
    })
    ...
    console.error('   ❌ 买入异常:', error.shortMessage || error.message)
    return null
  }
  ...
  try {
    ...
    console.error('   ❌ 授权异常:', error.shortMessage || error.message)
    return null
  }
  
  // 3. 卖出 (卖出一半)
  console.log('💸 步骤 3: 卖出 (卖出一半, 5% 滑点)')
  try {
    ...
    const sellAmountOutMin = sellAmountsOut[1] * BigInt(9500) / BigInt(10000) // 5% slippage
    console.log(`   最小获得 (5%): ${formatUnits(sellAmountOutMin, 18)} BNB`)
    ...
    console.error('   ❌ 卖出异常:', error.shortMessage || error.message)
  }
  
  // 4. 添加流动性 (0.0001 BNB)
  console.log('➕ 步骤 4: 添加流动性 (0.0001 BNB, 5% 滑点)')
  try {
    ...
    const amountTokenMin = tokenAmount * BigInt(9500) / BigInt(10000) // 5% slippage
    const amountETHMin = addEthAmount * BigInt(9500) / BigInt(10000)
  ...
  return { symbol, decimals, totalGasUsed }
}

async function main() {
  console.log('╔════════════════════════════════════════════╗')
  console.log('║  主网完整测试 - 5% 滑点 (支持 3-5% 税)     ║')
  console.log('╚════════════════════════════════════════════╝\n')
