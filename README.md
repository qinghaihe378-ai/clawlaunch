...
- server：后端（Express + MongoDB + Redis + Viem）
...
- VITE_API_BASE_URL：后端接口地址，开发环境可填 `http://localhost:3001/api`
...
- PORT：后端端口，默认 `3001`
- API_FACTORY_VERSION：当前启用的工厂版本，默认 `v1`
- FACTORY_V1_ADDRESS_BSC / FACTORY_V1_ADDRESS_BSC_TESTNET：可覆盖部署文件中的工厂地址
- BNB_RPC_URL / BNB_TESTNET_RPC_URL：后端读取链上数据使用的 RPC
- MONGODB_URI / MONGODB_DB_NAME：MongoDB 快照存储
- REDIS_URL：Redis 缓存
- CACHE_TTL_SECONDS：首页与详情缓存秒数
...
GET /api/health
GET /api/factories
GET /api/tokens?version=v1&chainId=56&page=1&pageSize=20
GET /api/tokens/:tokenAddress?version=v1&chainId=56
```
